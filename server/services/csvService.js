...
function formatDataForAPI(data) {
    return {
        ...
        campaignName: "update_mobile_app_api_prod_campaign",
        destination: "91" + data["Mobile Number"],  // Assuming "Mobile Number" is the key
        userName: data["Agent's Name"],
        source: "CSV import",
        templateParams: ["$AgentName", "$AgentName", "$StoreId", "$AgentId", "$StoreManagerName"],
        tags: ["AgentName", "AgentName", "StoreId", "AgentId", "StoreManagerName"],
        attributes: {
          "AgentName": data["Agent's Name"],
          "StoreId": data["Store ID"],
          "AgentId": data["Agents's ID"],
          "StoreManagerName": data["Manager Name"]
        }
      };
}
